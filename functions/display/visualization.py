...
def produce_movie_multi(Xs, evolve, interpolation=DEFAULT_INTERPOLATION):
    ...
    # Dimensions optimisées pour une résolution d'écran large
    menu_width = 350         # Augmenté de 300 à 350 pour plus d'explications
    ...
    total_width = min(2000, menu_width + sim_width + matrix_width)
    
    if total_width > 2000:
        available_width = 2000
        menu_width = int(available_width * 0.22)  # Augmenté de 0.20 à 0.22
        matrix_width = int(available_width * 0.28)  # Réduit légèrement de 0.30 à 0.28
